...
export default function GymHomeScreen() {
    ...
    return (
        <View style={cs.root}>
            ...
            <LinearGradient
                colors={['#1A1A1A', '#37474F', '#4E342E']}
                style={cs.header}
            >
                <Text style={cs.headerTitle}>SlotB Gym</Text>
            </LinearGradient>
            <View style={cs.body}>
                <Text style={cs.emoji}>🏋️</Text>
                <Text style={cs.title}>Coming Soon</Text>
                <Text style={cs.sub}>{"Get ready to lift!\nGym booking & membership will be live shortly."}</Text>
                <View style={cs.badge}>
                    <Text style={cs.badgeText}>Day Pass · Memberships · Personal Training</Text>
                </View>
            </View>
        </View>
    );
...
const cs = StyleSheet.create({
    ...
    headerTitle: { fontSize: 20, fontWeight: '900', color: '#FFD740', letterSpacing: 0.3 },
    body: { flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 32, gap: 14 },
    emoji: { fontSize: 64, marginBottom: 8 },
    title: { fontSize: 32, fontWeight: '900', color: '#1A1A1A', letterSpacing: 0.5 },
...
// ─────────────────────────────────────────────────────────────────────────────
// ORIGINAL GYM HOME SCREEN CODE — preserved below, commented out (Coming Soon)
// ─────────────────────────────────────────────────────────────────────────────
...
//             <SectionHeader title="Gym" scrollY={scrollY} searchPlaceholder="Search gyms, trainers..." accentColor="#FFD740" gradientColors={['#1A1A1A', '#37474F', '#4E342E']} />
